...
function init() {
  ...
  /* Reel previews. Videos and GIFs are loaded on demand. */
  ...
  function drawFrame(work, img) {
    ...
    if (img && (img.videoWidth || img.width)) {
      ...
      let fw, fh;
      const pad = work.vertical ? 1.15 : 1.0;
  ...
  async function select(i, instant) {
    ...
    /* draw black screen while loading video/gif */
    paintBackdrop(null);
    drawFrame(w, null);
    updateSpill();
    ...
    /* only video elements have play(); images (GIFs) don't need it */
    if (v.play) {
  ...
  if ('IntersectionObserver' in window) {
    new IntersectionObserver((es) => {
      ...
      if (live) { 
        if (live.play) { 
          if (inView) { const p = live.play(); if (p && p.catch) p.catch(() => {}); } else live.pause(); 
        }
      }
  ...
  document.addEventListener('visibilitychange', () => {
    if (!live) return;
    if (!live.pause || !live.play) return;
  ...
  function tick() {
    ...
    if (live) {
      const isVideo = live.play && live.paused !== undefined;
      if (isVideo) {
        if (!live.paused && !live.ended && live.readyState >= 2) {
          drawFrame(WORKS[current], live);
          updateSpillThrottled(t);
        }
      } else {
        /* GIF image — draw every frame */
        drawFrame(WORKS[current], live);
        updateSpillThrottled(t);
      }
    }
